...
retry:
  ...
  if (onfinish) {
     ...
     crf.reset(new
       Client_Read_Finisher(this, onfinish, onuninline.get(), iofinish.get(),
...
class Client_Read_Async_Finisher : public C_SaferCond {
public:
			...
			Context *onfinish)
{
  ...
  int r = 0;
  std::unique_ptr<C_SaferCond> io_finish = nullptr;
  if (onfinish == nullptr)
    io_finish.reset(new C_SaferCond("Client::_read_async flock"));
  else
